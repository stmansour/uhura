...
func TestResourceParsing() {
	...
	if UEnv.Instances[0].Resources.RestoreMySQLdb != "testdb.sql" {
		fmt.Printf("Resource callout for RestoreMySQLdb is wrong, expected testdb.sql, found %s", UEnv.Instances[0].Resources.RestoreMySQLdb)
		os.Exit(2)
	}
