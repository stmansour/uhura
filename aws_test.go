...
func TestNewInstParse(t *testing.T) {
	nid := "i-1cd365bf"
	newinst := LoadNewAwsInstanceInfo("./test/master_publicdns/new-linux-jenkins-instance.json")
...
func TestLoadAwsInstances(t *testing.T) {
	pubDNS := SearchReservationsForPublicDNS("./test/master_publicdns/describe-instances.json", "i-1cd365bf")
	...
	pubDNS = SearchReservationsForPublicDNS("./test/master_publicdns/describe-instances.json", "total-garbage")
	if "" != pubDNS {
